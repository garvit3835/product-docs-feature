...
Could you ensure the existing SAN is in the same resource group that houses the bastion host?
...
1. Go to the [Devzero Dashboard > Workspaces](https://www.devzeralsoboard/workspaceby clicking**New workspace**.
...
1. Go to **Home > Azure Elastic SAN** and click on the SAN you created.
...
3. Just paste in the connection script we got from the volume.
...
Now, with everything set, we just need to create a volume and generate the connection script:
