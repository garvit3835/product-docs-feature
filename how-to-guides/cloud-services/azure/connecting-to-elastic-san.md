...
Ensure that the existing SAN should be in the same resource gorup that houses the bastion host.
...
1. Go to **Home > Azure Elastic SAN** and click on the SAN you just created.
...
5. Now select the **Virtual network** and choose an available subnet.
6. Select **Yes** for **Integrate with private DNS zone** option, and click on **Ok**.
7. After creating the endpoint, click on **Create**.

{% hint style="info" %}
You can choose to add your VNET as well by click on **Create** under the **Virtual Networks** section and entering the details of the VNET which exists in the same region as the Elastic SAN instance.
{% endhint %}

### Step 4: Creating a Volume

Now with everything setup, we just need to create a volume and generate the connection script:
...
4. Click on **Create** and wait for it to intialize.
5. After the volume is created, select the volume and click on **Connect**.
6. Note down the scipt code for **Linux** as we will use it to make a connection between Azure infrastructure and the DevBox.

### Step 5: Connecting to the Elastic SAN from DevBox

Now you just need to follow the below steps to connect to DevBox:

1. Connect to the workspace from your terminal.
2. Create a python scipt by using the following command:
...
3. Now just paste in the connection scipt we got from the volume.
...
4. You can then allocate the desired SAN size in the **Resource provisioning on Elastic SAN** section.
...
{% hint style="info" %}
You can also choose the **Enabled** option and connect to a VNET if you have a VNET situated in the same **Region** as the Elastic SAN instance.
{% endhint %}
